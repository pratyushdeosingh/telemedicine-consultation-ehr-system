...
- [x] Confirm the daily keepalive appears enabled in Vercel Cron Jobs with schedule `0 6 * * *` UTC.
- [x] Run the Vercel cron job manually and confirm it succeeds.
- [ ] Inspect the first automatic scheduled invocation after the next `06:00`–`06:59` UTC window.
